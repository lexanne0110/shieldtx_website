...
function validateRequestAccess(body) {
  ...
  const data = {
    ...
    protection,
    urgency,
    // Optional API-interest checkbox → intended usage. Unchecked = Trading App.
    use_type: body.api_interest ? 'API' : 'Trading App',
  };
...
function validateSupport(body) {
  ...
  const subject = clampString(body.subject, 150);
  if (subject.length < 2) return { ok: false, error: 'Enter a subject.', field: 'subject' };

  const details = clampString(body.details, 4000);
  if (details.length < 2) return { ok: false, error: 'Add a few details.', field: 'details' };

  return { ok: true, data: { email: email.toLowerCase(), subject, details } };
}
